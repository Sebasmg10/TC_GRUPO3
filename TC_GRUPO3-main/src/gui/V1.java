...
public class V1 extends JFrame implements ActionListener {
			...
			}
			{
				...
				btnBUSCAR.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						try {
							if (ar.Buscar(Leercodigo(), true) == true) {
							JOptionPane.showMessageDialog(null, "Se encontró el curso dentro de la base de datos");
							} else {
	...
	protected void do_bTNMODIFICAR_actionPerformed(ActionEvent e) {
		try 
		{
			Nrc n = ar.Buscar(Leercodigo());
			if (n != null) 
			{
			n.setCant_max(Leercantmax());
			n.setCant_actu(Leercantact());
			n.setNombreCurso(LeerNombreCurso());
			n.setDocente(Leerdatosdocente());
			JOptionPane.showMessageDialog(null, "Se Modificó Correctamente");
			txts.setText("");
			
			}else {
				JOptionPane.showMessageDialog(null, "El NRC no existe");
			}
			}
			catch(Exception exx)
			{
